...
export async function puppet(
  ...
  try {
    ...
    try {
      ...
      // Wait for and download the generated image
      console.log("Waiting for image generation to complete...");

      // Wait for the final gallery to be visible and contain an image
      console.log("Waiting for image to appear in the gallery...");
      try {
        await page.waitForSelector("#final_gallery img", { timeout: 120000 });
        console.log("Image detected in gallery");

        // Get the image source URL
        const imageSrc = await page.evaluate(() => {
          const img = document.querySelector("#final_gallery img");
        ...
        if (imageSrc) {
          ...
          console.log("Image successfully downloaded to:", downloadPath);
        } else {
        ...
        try {
          const errorScreenshotPath = path.join(
            __dirname,
            "../../temp",
            `download_error_${new Date().getTime()}.png`
          );
    ...
    // Wait longer to see the result before closing
